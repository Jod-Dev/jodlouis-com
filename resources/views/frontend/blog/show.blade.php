                    ...
                    <div class="pb-5">
                       @if (Session::get('choose_version_key') != null)
                            @if (Session::get('choose_version_key') == 1)
                                @if (isset($previous)) <a class="float-left text-dark  w-40 text-left" href="{{ url('blog/'.$previous->slug) }}"><i class="fa fa-angle-left"></i> {{ $previous->title }}</a> @endif
                                @if (isset($next)) <a class="float-right text-dark w-40 text-right" href="{{ url('blog/'.$next->slug) }}">{{ $next->title }}  <i class="fa fa-angle-right"></i></a> @endif
                            @else
                                @if (isset($previous)) <a class="float-left text-white w-40 text-left" href="{{ url('blog/'.$previous->slug) }}"><i class="fa fa-angle-left"></i> {{ $previous->title }}</a> @endif
                                @if (isset($next)) <a class="float-right text-white w-40 text-right" href="{{ url('blog/'.$next->slug) }}">{{ $next->title }}  <i class="fa fa-angle-right"></i></a> @endif
                            @endif
                           @else
